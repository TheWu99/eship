...
async def generate_customs_form(
    shipment: Shipment, form_type: str = Query("CN22", description="Form type: CN22 or CN23")
):
    """
    ...
    if form_type not in ["CN22", "CN23"]:
        raise HTTPException(status_code=400, detail="Invalid form type. Must be 'CN22' or 'CN23'")

    try:
        form_pdf = await customs_service.generate_customs_form(shipment, form_type)
        return {"form_type": form_type, "format": "PDF", "content": form_pdf}
...
async def generate_commercial_invoice(
    shipment: Shipment, invoice_number: str = Query(None, description="Optional invoice number")
):
    """
    ...
    """
    try:
        invoice_pdf = await customs_service.generate_commercial_invoice(shipment, invoice_number)
        return {
            "document_type": "commercial_invoice",
            "format": "PDF",
            "invoice_number": (
                invoice_number or shipment.customs.invoice_number if shipment.customs else None
            ),
            "content": invoice_pdf,
        }
    except ValueError as e:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error generating commercial invoice: {str(e)}"
        )
