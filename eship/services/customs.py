...
from ..models import CustomsForm, Shipment
...
class CustomsService:
    ...
    async def generate_commercial_invoice(
        ...
        if invoice_number or shipment.customs.invoice_number:
            c.drawString(1*inch, y, f"Invoice Number: {invoice_number or shipment.customs.invoice_number}")
            y -= 0.25*inch
        ...
        y -= 0.2*inch
        c.drawString(1*inch, y, f"{shipment.from_address.city}, {shipment.from_address.state} {shipment.from_address.postal_code}")
        y -= 0.2*inch
        ...
        y -= 0.2*inch
        c.drawString(1*inch, y, f"{shipment.to_address.city}, {shipment.to_address.state} {shipment.to_address.postal_code}")
        y -= 0.2*inch
    ...
    async def _generate_cn23(self, shipment: Shipment) -> str:
        ...
        # Certification
        y -= 0.3*inch
        c.drawString(1*inch, y, "I certify that the particulars given in this declaration are correct.")
        y -= 0.3*inch
